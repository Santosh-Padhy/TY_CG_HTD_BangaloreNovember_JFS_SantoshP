...
public class CustomerDaoImpl implements CustomerDao {
	...
	public List<Customer> getCustomerinfo(int customerid) {
		String query = "SELECT * from product_info WHERE customer_id=?";
		try {
			preparedStatement = connection.prepareStatement(query);
			preparedStatement.setInt(1, customerid);
	...
	public boolean modifyCustomer(Customer customer) {
		...
		String updatequery = "Update customer_info SET customerid=?,customerName=?,streetAddress1=?,StreetAddress2=?,town=?,email=?,postalcode=?,telephoneNumber=? WHERE customerid=?";
		try {
